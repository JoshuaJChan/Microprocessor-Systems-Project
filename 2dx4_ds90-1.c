...
int main(void) {
  ...
  uint8_t dataReady;
	...
	FlashAllLEDs();				//Signals that ToF chip was booted
	...
	while(1){	//Keep polling for push button
		GPIO_PORTL_DATA_R = 0b000010000; 							//External LED on
		if((GPIO_PORTE_DATA_R & 0b00000001) == 0){ 		//If Button is pressed(Port E == 0)
			GPIO_PORTL_DATA_R = 0b000000000;						//Turn off External LED

			//Start Measurement
			for(int i = 0;i<8;i++){
				while (dataReady == 0){																	//Keeps polling while data is not ready
					status = VL53L1X_CheckForDataReady(dev, &dataReady);	//check if register is ready, else wait
					VL53L1_WaitMs(dev, 5);
				}
				dataReady = 0;																			//data variable reset to not ready
				status = VL53L1X_GetRangeStatus(dev, &RangeStatus);	//Returns whether the range measurement was successful or not
				status = VL53L1X_GetDistance(dev, &Distance);				//Returns the distance in mm
				stepper();
				status = VL53L1X_ClearInterrupt(dev); 							//clear interrupt has to be called to enable next interrupt
				sprintf(printf_buffer,"%u, %u, %u, %u, %u\r\n", RangeStatus, Distance, SignalRate, AmbientRate,SpadNum); 
				UART_printf(printf_buffer);													//outputs is captured by Python program
			}
			GPIO_PORTK_DATA_R = 0b0000;														//turn off outputs to stepper motor
		}
	}
	//disable ranging
...
void stepper() {
	uint32_t stage[] = {0b1001,0b1100,0b0110,0b0011};					//Sequences of turning stepper motor
	for(int i = 0;i<64*4;i++){	//i is counter of turns, 64 turns:1 cycle for 11.25deg --> 64 turns * 4 stages for one cycle aka 1/8 a revolution
		GPIO_PORTK_DATA_R = stage[i%4];
		SysTick_Wait(200000);  																	
	}
	GPIO_PORTF_DATA_R = 0b0001;																//turn on onboard LED
	SysTick_Wait(1200000);  																	
	GPIO_PORTF_DATA_R = 0b0000;																//turn off onboard LED
}
...
void Ports_Init() {	//***I added this in
	...
	//init ports
	GPIO_PORTK_DIR_R = 0b00001111;      //PK[0-3] output for stepper motor
